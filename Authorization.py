...
class Authorization:
    ...
    def get_callback(self, request):
        ...
        """
        logging.info("get_callback: Request: %s\n  request.args: %s\n  request.url: %s session['state']: %s",
                    request, request.args, request.url, session["state"])
        state = session["state"]
        flow = self.get_flow()
        flow.fetch_token(authorization_response=request.url)
        logging.info(f"get_callback: flow.credentials: {flow.credentials}\n")
        if not state == request.args["state"]:
            return "State does not match!", 400

        credentials = flow.credentials
        logging.info(f"get_callback: flow.credentials: {credentials.to_json()}\n")
